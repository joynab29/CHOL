...
// Handle deleting an option from the session
if (isset($_POST['delete_option'])) {
    $index = $_POST['option_index'];
    if (isset($_SESSION['options'][$index])) {
        unset($_SESSION['options'][$index]); // Delete the selected option
        $_SESSION['options'] = array_values($_SESSION['options']); // Reindex the array
    }
}
?>
...
<head>
    ...
    <script>
        function addOption() {
            ...
            // Create a delete button for each option
            var deleteButton = document.createElement('button');
            deleteButton.innerText = 'Delete';
            deleteButton.classList.add('delete-btn');
            deleteButton.onclick = function() {
                deleteOption(newOptionDiv); // Call delete function when clicked
            };
        ...
        function deleteOption(optionDiv) {
            optionDiv.remove(); // Remove the selected option div
        }
    </script>
</head>
<body>
    ...
    <div class="poll-form-container">
        ...
        <form action="create_poll.php" method="POST">
            ...
            <div id="options-container">
                <?php
                    if (isset($_SESSION['options'])) {
                        foreach ($_SESSION['options'] as $index => $option) {
                            $formatted_budget = number_format($option[1], 2);  // Format budget as currency
                            echo '<div class="option-group">
                                    <input type="text" value="' . htmlspecialchars($option[0]) . '" readonly>
                                    <input type="text" value="$' . $formatted_budget . '" readonly>
                                    <button type="button" class="delete-btn" onclick="deleteOption(this.parentElement)">Delete</button>
                                  </div>';
                        }
                    }
                ?>
